...
function Login() {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      navigate("/User");
      window.alert(` ${auth.currentUser.displayName} Welcome !`);
    } catch (error) {
      window.alert("There is a problem signing in with google. ");
    }
  };
  const singIn = async () => {
    if (login === "" || password === "") {
      window.alert("Please Enter Your Password And Email");
      return;
    }
    try {
      ...
      navigate("/User");
      window.alert(
        `Signed In Successfully ! Welcome ${auth.currentUser.displayName}`
      );
    } catch (error) {
      window.alert("Please check login and password provided above");
    }
  };
  return (
    <div className="flex flex-row pb-10 xl:max-h-[100vh]  xl:overflow-hidden">
      ...
      <div className="mx-auto mt-10 flex min-h-[95vh] w-[70%] flex-col items-center space-y-16 xl:min-w-[45vw] xl:space-y-14">
        ...
        <button
          ...
          className="flex items-center justify-between rounded-full border bg-white px-[3vw] py-[2.5vw] text-sm drop-shadow-2xl sm:px-6 sm:py-4 sm:text-base"
        >
          <img
